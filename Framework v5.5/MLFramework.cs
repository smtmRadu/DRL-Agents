...
namespace MLFramework
{
    ...
    public class NeuralNetwork
    {
        ...
        //used for momentum ** adding this in a future update
        float[][][] weightsVelocities = null;
        float[][] biasesVelocities = null;

        float samplesError = 0;
        public void UpdateGradients(float[] inputs, float[] desiredOuputs, LossFunctionType lossFunction)
        {
            //learn rate keeps between [0,1] here
            ...
            //Update gradients for last weights
            samplesError += CalculateOutputNodesCost(outputs, desiredOuputs, lossFunction);
        ...
        public void ApplyGradients(float learnRate, int batchSize)
        {
        ...
        private void ApplyGradientsOnLayer(int weightLayerIndex, float modifiedLearnRate)
        {
            ...
            for (int i = 0; i < outNeurons; i++)
            {
                ...
                {
                    weights[weightLayerIndex][i][j] -= weightsGradients[weightLayerIndex][i][j] * modifiedLearnRate; // + velocity * momentum
                    weightsGradients[weightLayerIndex][i][j] = 0;//reset gradients
                }
                biases[biasLayerIndex][i] -= biasesGradients[biasLayerIndex][i] * modifiedLearnRate;
                biasesGradients[biasLayerIndex][i] = 0;
            }
        }
        private void UpdateGradientsForLayer(int weightLayerIndex)
        {
            ...
            for (int i = 0; i < outNeurons; i++)
            {
                for (int j = 0; j < inNeurons; j++)
                {
                    weightsGradients[weightLayerIndex][i][j] += nodes[weightLayerIndex][j].valueOut * nodes[weightLayerIndex + 1][i].costValue;
                }
                biasesGradients[biasLayerIndex][i] += 1 * nodes[weightLayerIndex + 1][i].costValue;
                //is weightLayerIndex+1 because the biasesGradients arr is not the same as for weights
            }
        }


        //calc output node values 
        ...
        void CalculateNodesCost(int neuronsLayer)
        {
            ...
            if (neuronsLayer == 0)
                return;
            int nodesNum = nodes[neuronsLayer].Length;
            ...
            {
                nodes[neuronsLayer][i].costValue = 0;
                for (int j = 0; j < nextLayerNeuronsNum; j++)
                {
                    nodes[neuronsLayer][i].costValue += nodes[neuronsLayer + 1][j].costValue * weights[neuronsLayer][j][i]; // sum of each nextNeuron*connectionWeight;
                }

                nodes[neuronsLayer][i].costValue *= ActivationDerivative(nodes[neuronsLayer][i].valueIn, false);
            }
        }
        private void InitMainGradients()
        {
            ///Initializes weightsGradients[][][] and biasesGradients[][] from null to zeroing all
            /////first array of biases are not counted because they belong to input layer neurons
        ...
        public float GetError()
        {
            ...
            float whatToReturn = samplesError;
            samplesError = 0;
            return whatToReturn;
        }
        //------------------------------------------------------------------------END HEURISTIC ------------------------------------------------------------------------------//
