...
namespace MLFramework
{
    public class NeuralNetwork
    {
        ...
        protected void Initialize(float[] axons)
        {
            ...
            switch (initialization)
            {
                case InitializationFunctionType.StandardNormal1:
                    ...
                    {
                        axons[i] = InitializationFunctionStandardNormal(0.15915f, 2f, 0.3373f);
                    }
                    break;
                case InitializationFunctionType.StandardNormal2:
                    for (int i = 0; i < axons.Length; i++)
                    {
                        axons[i] = InitializationFunctionStandardNormal(0.15915f, 0.2f, 0.3373f);
                    }
                    break;
                case InitializationFunctionType.StandardNormal3:
                    for (int i = 0; i < axons.Length; i++)
                    {
                        axons[i] = InitializationFunctionStandardNormal(0.15915f, 2f, 0.722f);
                    }
                    break;
        ...
        static float InitializationFunctionStandardNormal(float l, float k, float z)
        {
        ...
        void ChaoticMutation(ref float weight)
        {
            ...
            if (chance < .125f)
                weight = InitializationFunctionStandardNormal(0.15915f, 2f, 0.3373f);
            else if (chance < .3f)
        ...
        static float Activate(float value, bool outputLayer = false)
        {
            ...
            if (outputLayer)
                where = outputActivation;
            else where = activation;
        ...
        float CalculateOutputNodesCost(float[] outputs, float[] expectedOutputs, LossFunctionType lossfunc)
        {
            ...
            if (lossfunc == LossFunctionType.Quadratic)
                for (int i = 0; i < outputs.Length; i++)
                {
                    ...
                    cost += localCost;
                    nodes[nodes.Length - 1][i].costValue = QuadraticNodeCostDerivative(outputs[i], expectedOutputs[i]) * ActivationDerivative(nodes[nodes.Length - 1][i].valueIn, true);
                }
                ...
                for (int i = 0; i < outputs.Length; i++)
                {
                    ...
                    cost += localCost;
                    nodes[nodes.Length - 1][i].costValue = AbsoluteNodeCostDerivative(outputs[i], expectedOutputs[i]) * ActivationDerivative(nodes[nodes.Length - 1][i].valueIn, true);
                }
                ...
                for (int i = 0; i < outputs.Length; i++)
                {
                    ...
                    nodes[nodes.Length - 1][i].costValue = CrossEntropyNodeCostDerivative(outputs[i], expectedOutputs[i]) * ActivationDerivative(nodes[nodes.Length - 1][i].valueIn, true);
                }
            return cost;
        }//LOSS FUNCTION
    ...
    public class AgentBase : UnityEngine.MonoBehaviour
    {
        ...
        [Range(0, 180), Tooltip("Heuristic Training length in seconds.")] public float sessionLength = 60;
        ...
        void HeuristicAction()
        {
            ...
            if (sessionLength < 0)
            {
                Debug.Log("Heuristic training session ended. You can start another session by changing the behavior to Heuristic.");
            ...
            float normalizedLearnRate = learnRate * Time.deltaTime;
            network.Learn(inputs.GetBuffer(), desiredOutputs.GetBuffer(), normalizedLearnRate, costType);

            error = network.GetOverallCost();
