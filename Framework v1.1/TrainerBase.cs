...
//This TrainerBase class works like this
/*Takes all AI's and the best brain 
 * every episodesPerEvolution* times, the brain with the highest fitness is copyied in BestNeuralNetwork
 * the nextGeneration get the best brain that is now and a mutation is applied for each of them
 * the procces repeats
 */
//The improved TrainerBase class works like this -> Uses NextGeneration2
/*Takes all AI's and give them this brain
 * half of them are mutated
 * every episodesPerEvolution* times, the best half brains are saved in a Directory called Best_Neural_Networks  [also the best brain from all of them is saved in Best_Neural_Network]
...
public class TrainerBase : MonoBehaviour
{
    ...
    [SerializeField] TMPro.TMP_Text statisticsDisplay = null;
    ...
    bool canTrain = true;//Var used for checking all the requirements in order to train
    bool callTraining = true;//Var used to call Training() ->was implemented to get rid of a potential stack overflow
    ...
    void Update()
    {
        if (canTrain == true && callTraining == true)
    ...
    protected virtual void SetupTeam()
    {
        ...
        for (int i = 0; i < teamSize; i++)
        {
            ...
            teamArray[i].controller = member.GetComponent<Agent>() as Agent;
