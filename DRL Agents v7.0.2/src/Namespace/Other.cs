...
namespace DRLAgents
{
    ...
    internal readonly struct Functions
    {
        ...
        {
            static public float BinaryStep(float value)
            {
                if (value < 0)
                    return 0;
                else return 1;
            }
            static public float Sigmoid(float value)
            {
                //values range [0,1]
            ...
            static public float ReLU(float value)
            {
                return Mathf.Max(0, value);
            }
            static public float LeakyReLU(float value, float alpha = 0.2f)
            {
                if (value > 0)
                    return value;
                else return value * alpha;
            }
            static public float SiLU(float value)
            {
                return value * Sigmoid(value);
            }
            static public void SoftMax(ref float[] values)
            {
                float sum = 0f;
        ...
        internal readonly struct Derivatives
        {
            ...
            {
                float[] valuesIn = new float[values.Length];
                Array.Copy(values, valuesIn, values.Length);

                Functions.Activation.SoftMax(ref valuesIn);//values in are now softmaxed
        ...
        internal readonly struct Cost
        {
            ...
            {
                return -expectedOutput * Mathf.Log(outputActivation);
            }
            static public float CrossEntropyDerivative(float outputActivation, float expectedOutput)
            {
                return -expectedOutput / outputActivation;
            }
        }
        ...
        internal readonly struct Initialization
        {
            ...
            static public float RandomInNormalDistribution(float mean = 0, float standardDeviation = 1)
            {
    ...
    public enum ActivationFunctionType
    {
        ...
        [Tooltip("@output: [0, 1]\n" +
                 ...
                 "@good for decisional output (one output at a time -> there can be multiple actions in a single output)")]
        SoftMax,
